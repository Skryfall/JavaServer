...
def p_statement_main(p):
    """statement : Begin Game1 BeginParentesis initializer add game1 EndParentesis Game2 BeginParentesis initializer add game2 EndParentesis Finish Semicolon"""
    print(names)
...
def p_add_toList(p):
    ...
    array = names[p[1]][0]
    if isinstance(array[0], str) and isinstance(p[6], str) and p[3] < array[2] and len(p[6]) < array[1] + 3:
        names[p[1]][p[3] + 1] = p[6]
    elif isinstance(array[0], int) and isinstance(p[6], int) and p[3] < array[1]:
        names[p[1]][p[3] + 1] = p[6]
    else:
